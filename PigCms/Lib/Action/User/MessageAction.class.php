...
class MessageAction extends UserAction{
	...
	public function tosendAll(){
		if (IS_GET){
			...
			//
			//根据openid群发接口
			$userids = json_decode($this->curlGet('https://api.weixin.qq.com/cgi-bin/user/get?access_token='.$postMedia['access_token']));
			if (!$json->errmsg){
				$userlist = $userids->data->openid;
			}
			...
			$sendrt=$this->curlPost('https://api.weixin.qq.com/cgi-bin/message/mass/send?access_token='.$postMedia['access_token'],'{"touser":'.$userlist_str.',"mpnews":{"media_id":"'.$row['mediaid'].'"},"msgtype":"mpnews"}');
			if($sendrt['rt']==false){
