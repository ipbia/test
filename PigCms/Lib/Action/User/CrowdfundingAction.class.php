...
class CrowdfundingAction extends UserAction
{
	...
	public function index()
	{
		...
		foreach ($list as $key => $val) {
			...
			$info['price_count'] = $this->getOrderCount('', 1);
			$info['people_count'] = $this->getOrderCount();
	...
	public function getOrderCount($reward_id = '', $type = '')
	{
		...
		if ($type == '') {
			...
			$count = M('Crowdfunding_order')->where($where)->sum('price');
		}
