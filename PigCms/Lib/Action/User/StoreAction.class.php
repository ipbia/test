...
public function twitter()
	...
	public function noticeAdd()
	{
		if (IS_POST) {
			$_POST['token'] = session('token');
			if (D('Product_notice')->add($_POST)) {
				$this->success('修改成功', U('Store/notice', array('token' => session('token'))));
			}
		}else{
			$this->display('noticeSet');
		} 
	}
	...
	public function noticeSet()
	{
		...
		$checkdata = M('Product_notice')->where($where)->find();
		if(empty($checkdata)){
			$this->error("没有相应记录.您现在可以添加.", U('Store/noticeAdd', array('token' => session('token'))));
		}
		if (IS_POST) {
			$data = D('Product_notice');
			if ($data->create()) {
	...
	public function noticeDel()
	{
		...
		if (IS_GET) {
			...
			$back = $data->where($where)->delete();
			if ($back == true) {
				$this->success('操作成功', U('Store/notice', array('token' => session('token'))));
			} else {
				$this->error('服务器繁忙,请稍后再试', U('Store/notice', array('token' => session('token'))));
			}
		}
	}
