...
class UserAction extends BaseAction{
	...
	public function canUseFunction($funname, $return = false){
		//权限验证
		...
		$token_open=M('token_open')->field('queryname')->where(array('token'=>$this->token))->find();
		if($token_open != false){
			if(!strpos(strtolower($token_open['queryname']), strtolower($funname))){
				$token_open = false;
			}else{
				$token_open = true;
			}
		...
		if (in_array(strtolower($funname),$user_group) === false || !$allow || !$token_open){
			if($return == false){
