...
class HelpingAction extends WapAction{
		...
		if($this->fans){
			...
			if(empty($user)){
				...
				M('Helping_user')->add($data);
							/*分享失效备用*/
				if($this->wecha_id && empty($share_key)) {
					redirect(U('Helping/index',array('token'=>$this->token,'id'=>$id,'share_key'=>$user['share_key'])));
				}
			}else{
	...
	public function add_share(){
		...
		$is_share 	= M('Helping_record')->where(array('token'=>$this->token,'pid'=>$share['pid'],'share_key'=>$share_key))->count();

		if(in_array($share_key, $cookie_arr[$this->helping['id']])) {
