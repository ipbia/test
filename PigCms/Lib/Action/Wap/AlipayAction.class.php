...
class AlipayAction extends BaseAction{
	...
	public function redirect_pay(){
		...
		if($alipayConfig){
			...
			foreach($config_info as $key=>$value){
				if(is_array($value)){
					if($value['open'] && strtolower($key) == strtolower($pay_type)){
						$key = ($key == 'alipay') ? 'Alipaytype' : ucwords($key);
						$pay_setting[$key] = $value;
						$now_pay_type = $key;
					}
		...
		if($now_pay_type == 'Weixin'){
			header('Location:/wxpay/index.php?g=Wap&m=Weixin&a=pay&price='.$_GET['price'].'&orderName='.$_GET['orderName'].'&single_orderid='.$_GET['single_orderid'].'&showwxpaytitle=1&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id']);
		}else{
			header('Location:/index.php?g=Wap&m='.$now_pay_type.'&a=pay&price='.$_GET['price'].'&orderName='.$_GET['orderName'].'&single_orderid='.$_GET['single_orderid'].'&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&platform='.intval($platform));
		}
	}
}
?>