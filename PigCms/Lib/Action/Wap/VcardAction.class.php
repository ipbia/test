...
class VcardAction extends BaseAction{
	...
	public function index(){
		...
		$info = M('Vcard_list')->where($where)->find();
	...
	public function chl($id,$token,$wecha_id){
	   ...
	   //$margin = 0;
	   $size = "100";
	   return '<img src="https://chart.googleapis.com/chart?chs='.$widhtHeight.'x'.$widhtHeight.'&cht=qr&chld='.$EC_level.'|'.$margin.'&chl='.urlencode($chl).'" alt="QR code" widhtHeight="'.$size.'" widhtHeight="'.$size.'" width="230px" height="230px;"/>';
	}
